            ...
            if (
                ...
                for hyp in hyps:
                    if len(hyp) > 10:
                        ...
                        code = code.clip(max=emb_count - 2)
...
def dataio_prepare(hparams, run_opts):
    """This function prepares the datasets to be used in the brain class.
    It also defines the data processing pipeline through user-defined functions.
    """
    feats_cache_dir = pl.Path(hparams["feats_cache_dir"])
    cache_mode = "r" if (feats_cache_dir / "tgt_codes.hdf5").exists() else "a"
    kmeans_model = get_kmeans(hparams)
    device = run_opts.device
    tgt_encoder = hparams["tgt_encoder"].to(device)
    layer = hparams["layer"]
    ...
    @CachedHDF5DynamicItem.cache(
        feats_cache_dir, cache_filename="tgt_codes.hdf5", file_mode=cache_mode
    )
    @sb.utils.data_pipeline.takes("id", "tgt_sig")
    @sb.utils.data_pipeline.provides("code")
    def unit_pipeline(utt_id, sig):
        sig = sig.unsqueeze(0).to(device)
        feats = tgt_encoder.extract_features(sig)
        feats = feats[layer].squeeze(0).cpu().numpy()
        code = kmeans_model.predict(feats)

        code = torch.as_tensor(code, dtype=torch.long)
        code = torch.unique_consecutive(code)
        return code

    @sb.utils.data_pipeline.takes("id", "code")
...
if __name__ == "__main__":
    ...
    datasets, train_bsampler = dataio_prepare(hparams, run_opts)
